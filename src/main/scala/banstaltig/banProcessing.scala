...
object banProcessing {
  val banConfig = Configuration("bannie", "svowdb20.ivdnt.loc","banlijst", "postgres", "inl")
  lazy val banBase = new Database(banConfig)

  object queryBuilding {

    def valueClause(fieldName: String): String =
      s"""(select entity_id, '$fieldName' as name, cast(field_${fieldName}_value as text) as value,
    ...
    lazy val objectProperties = List("continent","endoniem","hoort_bij", "land","synoniemen","zie_ook")

    lazy val dataProperties = List("taal", "afkorting", "indeling", "iso_3166_alpha_2", "iso_3166_alpha_3",
      "iso_3166_numeriek", "map_embed", "opties", "taal", "transcriptie_nl", "transcriptie_vl", "voertaal", "voertalen")
  }


  import queryBuilding._

  val clauses = objectProperties.map(targetClause) ++
                List("body").map(bodyClause) ++
                dataProperties.map(valueClause)

  val query = s"create temporary table all_fields as ${clauses.mkString( " union ")};"

  val q1 =
    """drop table if exists bandata.overview;
      create table bandata.overview as select
          n1.*, node.title as target_title
      from (select node.title, node.type, node.language, all_fields.*
               from node, all_fields where node.nid=all_fields.entity_id) n1
               left join node on n1.target=node.nid;""".stripMargin

  val q2 =
    """drop table if exists bandata.grouped_overview;
       create table bandata.grouped_overview as select entity_id,max(type) as type,max(language) as language ,
       max(title) as title, array_agg(name || ':' || value) filter (where value is not null) as values,
       array_agg(name || ':' || target || ':' || target_title) filter (where target is not null) as linked_values
       from bandata.overview group by entity_id;""".stripMargin
