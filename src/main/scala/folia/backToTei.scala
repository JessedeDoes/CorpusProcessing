...
trait FoliaToRudimentaryTEITrait {

  val xml = "@{http://www.w3.org/XML/1998/namespace}"
  val posMapping: String => String = x => x
  val utterance_element:String="l"
  ...
  def convertWord(w: Node) = {
    ...
    val teiw = if (cls != "PUNCTUATION")
      <w xml:id={id} pos={posMapping(pos)} lemma={lemma}>
        {content}
      </w>
    else
      <pc xml:id={id} pos={posMapping(pos)}>
        {content}
      </pc>
  ...
  def convertUtterance(s: Node) = {
    ...
    <l xml:id={id}>
      {(s \\ "w").map(convertWord)}
    </l>.copy(label = this.utterance_element)
  }
...
case class FoliaToRudimentaryTEI(override val posMapping: String => String = x =>x, override val utterance_element:String="l") extends FoliaToRudimentaryTEITrait
{
