...
object Collocation
{
  ...
  def context(c: Concordance, w:Int, s:String, f:Option[Filter] = None):Seq[String] =
  {
    val a = c.tokenProperties(s)
    ...
    val filtered = indexes.filter(k => k > 0  && k < a.length)
    return filtered.map(k => a(k))
  }


  def streamContext(c: => Stream[Concordance], w:Int, p:String, f:Option[Filter] = None):Stream[String] =
  {
    c.flatMap(c => context(c,w,p,f))
  }

  def contextFrequencies(c: => Stream[Concordance], w:Int, p:String, f:Option[Filter]) =
    streamContext(c,w,p,f).groupBy(s => s).map({ case (k,str) => (k, str.foldLeft(0)( (c,s) => c+1)) }).toList

  def contextFrequencies(c: => Stream[Concordance], f:Option[Filter]): List[(String,Int)] = contextFrequencies(c, defaultWindow,"word",f)
