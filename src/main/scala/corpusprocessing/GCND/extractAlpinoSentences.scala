...
object ExtractAlpinoSentencesFromFolia  {
  ...
  def main(args: Array[String]) : Unit = {
    ...
    val files = new File(Settings.foliaDir).listFiles().iterator.filter(f => (XML.loadFile(f)  \\ "metadata" \\ "transcriptie_status" \ "label").text.contains("aalkundige verrijking geco"))
    files.foreach(f => {
