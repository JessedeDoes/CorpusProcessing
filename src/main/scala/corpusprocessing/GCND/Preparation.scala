...
object Preparation {
  ...
  val alpino_elan_stricter =
    """
      |drop view if exists elan_2_alpino cascade;
      |drop view if exists elan_2_alpino_unique;
      |
      |create index if not exists et_index on elan_annotatie(transcriptie_id);
      |create index if not exists at_index on alpino_annotatie(transcriptie_id);
      |
      |create temporary table elan_2_alpino_stricter as
      |        select e.transcriptie_id, elan_annotatie_id, alpino_annotatie_id, e.opname__persoon_id as pe, a.opname__persoon_id as pa,
      |        case
      ...
      |        transcriptie_id,
      |        alpino_annotatie_id,
      |        (array_agg(elan_annotatie_id order by pe=pa desc, overlap desc))[1] as elan_annotatie_id,
      |        array_agg(elan_annotatie_id order by pe=pa desc, overlap desc) as options,
      |        array_agg(overlap order by overlap desc) as overlaps from elan_2_alpino_stricter group by alpino_annotatie_id, transcriptie_id;
      |
      |""".stripMargin

  val link_elan_and_alpino_annotations=
    """
      |drop view if exists elan_2_alpino cascade;
      |drop view if exists elan_2_alpino_unique;
      |
