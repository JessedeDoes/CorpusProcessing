...
import corpusprocessing.brievenalsbuit.TagStuff._
import corpusprocessing.brievenalsbuit.TagStuff
import corpusprocessing.brievenalsbuit.LemPos
import utils.PostProcessXML._
import utils.{ProcessFolder, Tokenizer}

import scala.io.Source._
import scala.xml._

object gentse_spelen {
  ...
  def doW(w: Elem) =
  {
    val annotation = TagStuff.parseLemPos((w \ "@lemma").text, (w \ "@ctag").text.replaceAll("NE_","NE")).toTDN.map(x => x match
      {
        case LemPos(l,t) if
          ((w \ "@n").nonEmpty && t.contains("NOU-P") && Settings.addPartInfoInTag) => LemPos(l, t.replaceAll("\\)", ",wordpart=proper)"))
        case _ => x
      }
    )
   // println(annotation)
    ...
    */
    ...
    val fs: NodeSeq = Seq() // annotation.featureStructure

    w.copy(child=(if (Settings.discardSubElemsOfWord) <seg>{word}</seg> else <seg>{w.child}</seg>) ++ fs,
 ...
 // ook niet goed: Terug naar decimaal! 7 voor (7, zeveneneenhalf, TW) Dus altijd als deel dan blokkeren!
  ...
  def tokenize(w: Elem):Seq[Node] =
  {
    ...
    val safe = (!t.trailing.startsWith(".") || veiligePunten.contains(word,lemma)) && (Settings.discardSubElemsOfWord || seg.child.filter(_.isInstanceOf[Elem]).isEmpty)
    if (!safe || !nonTriv)
      w
    else
      {
  ...
  def doFile(in:String, out:String): Unit = {
    val d = XML.load(in)
    ...
    val d7 = TokenGroups(d6).markGroups()
    XML.save(out,d7,"UTF-8")
  }

  def main(args: Array[String]): Unit = {
    ProcessFolder.processFolder(Settings.input, Settings.output, doFile)
  }
}
