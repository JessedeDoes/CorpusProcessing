...
import corpusprocessing.gekaapte_brieven.Metadata.{meta, unify_unknown}
...
case class Participant(typ: String, fields: Map[String, String]) {
  ...
  lazy val xml = <person xml:id={personId} role={this --> "rol_xln"} gender={this --> "geslacht_xl"}>
    ...
    <occupation>{Metadata.unify_unknown(this -> List("beroep_gecorrigeerd_xln", "beroep_xln"))}</occupation>
    <event type={event}>
      <desc>{event}</desc>
