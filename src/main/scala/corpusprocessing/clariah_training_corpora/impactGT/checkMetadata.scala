...
import corpusprocessing.clariah_training_corpora.impactGT.checkMetadata.KBMeta
...
object checkMetadata {
  ...
  lazy val files = new File(pageDir).listFiles().iterator.filter(_.getName.endsWith(".xml")).map(x => x.getName -> XML.loadFile(x))
...
<meta key="Title">Arnhemsche courant</meta>
  ...
  case class PageMeta(e: Elem) {
    val fields = (e \\ "meta").map(m => {
      (m \ "@key").text -> m.text
    }).toMap

    val originalPath = fields("Original.Path")

    val id = originalPath.replaceAll(".*/", "").replaceAll("_(master|access).[a-z0-9]*$", "")

    def pad(x: String)  = if (x.length == 1) "0" + x else x
    val year = fields("Publication.Date.Year")
    val month = pad(fields.getOrElse("Publication.Date.Month","?"))
    val day = pad(fields.getOrElse("Publication.Date.Day","?"))
