...
object CRM2Xml {
  ...
  def linkWithOriginal(s: Stream[Document], currentOriginal: Document):Stream[(Document, Document)] =
  {
    if (s.isEmpty) Stream.empty
    else if (s.head.metadata.isEmpty)
      Stream.cons( (s.head, currentOriginal), linkWithOriginal(s.tail, currentOriginal))
    else Stream.cons( (s.head, s.head), linkWithOriginal(s.tail, s.head))
  }
  ...
  def process():Unit =
  {
    ...
    val doSomethingWithN:Stream[(Document, Document)] = linkWithOriginal(documents, documents.head)
    val xDocs = doSomethingWithN.map({case (d1,d2) => { d1.copy(metadata=Some(d2.me))}})
