...
class mapMiddelnederlandseTagsClass(gysMode: Boolean) {
  ...
  def patchPoSMistakes(m: String, p:String, w: String): String =
  {
    ...
    val p1 = if (p.startsWith("NUM") && p.contains("indef")) {
      ...
      if (st0 == "general") // dit is een nog twijfelachtige beslissing --- wanneer kan je dit doen?????
          s"""AA(position=adverbial,inflection=$infl)""" ;// nee inflectie is weg zo...
      else  s"""ADV(type=$typ,subtype=$subtype,inflection=$infl)"""
    }
...
*/
  ...
  def patchPosMistakes(morfcodes: List[String], pos: List[String], w: String) = {
     val patchedPos = morfcodes.zip(pos).map{case (a,b) => patchPoSMistakes(a,b,w)}
     patchedPos.mkString("+")
  }
